...
QString QFSFileEnginePrivate::longFileName(const QString &path)
{
	...
	if(absPath[1] == ':') return absPath;
	else return prefix + absPath;
#else
    return absPath;
#endif
}
...
bool QFSFileEnginePrivate::nativeOpen(QIODevice::OpenMode openMode)
{
    ...
    fileHandle = CreateFileA((const char*)fileEntry.nativeFilePath().utf8(),
                            accessRights,
                            shareMode,
                            &securityAtts,
                            creationDisp,
...
#if !defined(Q_OS_WINCE) && !defined(QT_NEWPORT)
                                 ...
                                 QFile::MemoryMapFlags flags)
{
    ...
    if (mapHandle == INVALID_HANDLE_VALUE) {
        ...
        mapHandle = ::CreateFileMappingA(handle, 0, protection, 0, 0, 0);
        if (mapHandle == INVALID_HANDLE_VALUE) {
