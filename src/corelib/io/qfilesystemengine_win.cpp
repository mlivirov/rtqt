...
extern UINT winSetErrorMode(
  __in  UINT uMode
);

BOOL winGetFileAttributesEx(
  __in   LPCSTR lpFileName,
  __in   GET_FILEEX_INFO_LEVELS fInfoLevelId,
  __out  LPVOID lpFileInformation
)
{
    HANDLE h = FindFirstFileA(lpFileName, (WIN32_FIND_DATAA *)lpFileInformation);
    FindClose(h);
...
static inline bool getFindData(QString path, WIN32_FIND_DATAA &findData)
{
    ...
    if (!path.endsWith(QLatin1Char(':'))) {
        HANDLE hFind = ::FindFirstFileA((const char*)path.utf8(), &findData);
...
QString QFileSystemEngine::nativeAbsoluteFilePath(const QString &path)
{
    ...
    QString absPath;
#if !defined(Q_OS_WINCE)
    QVarLengthArray<char, MAX_PATH> buf(qMax(MAX_PATH, path.size() + 1));
    char *fileName = 0;
    DWORD retLen = GetFullPathNameA((const char*)path.utf8(), buf.size(), buf.data(), &fileName);
    if (retLen > (DWORD)buf.size()) {
        buf.resize(retLen);
        retLen = GetFullPathNameA((const char*)path.utf8(), buf.size(), buf.data(), &fileName);
    }
    if (retLen != 0)
        absPath = QString::fromUtf8(buf.data(), retLen);
	else absPath = path;
#else
                                     ...
                                     QFileSystemMetaData::MetaDataFlags what)
{
    ...
    UINT oldmode = winSetErrorMode(SEM_FAILCRITICALERRORS | SEM_NOOPENFILEERRORBOX);
    if (GetFileInformationByHandle(fHandle , &fileInfo)) {
        data.fillFromFindInfo(fileInfo);
    }
    winSetErrorMode(oldmode);
    return data.hasFlags(what);
}
                                     ...
                                     QFileSystemMetaData::MetaDataFlags what)
{
    ...
    if (what & QFileSystemMetaData::WinStatFlags) {
        ...
        bool ok = ::winGetFileAttributesEx((const char*)fname.nativeFilePath().utf8(), GetFileExInfoStandard,
                                        &findData);
        if (ok) {
...
static inline bool mkDir(const QString &path)
{
    ...
    if (platformId == 1 && QFSFileEnginePrivate::longFileName(path).size() > 256)
        return false;
#endif
    return ::CreateDirectoryA((const char*)QFSFileEnginePrivate::longFileName(path).utf8(), 0);
}

static inline bool rmDir(const QString &path)
{
    return ::RemoveDirectoryA((const char*)QFSFileEnginePrivate::longFileName(path).utf8());
}
...
static bool isDirPath(const QString &dirPath, bool *existed)
{
    ...
    DWORD fileAttrib = ::GetFileAttributesA((const char*)QFSFileEnginePrivate::longFileName(path).utf8());
...
QString QFileSystemEngine::tempPath()
{
    ...
    if (len)
        ret = QString::fromUtf8(tempPath, len);
    if (!ret.isEmpty()) {
...
bool QFileSystemEngine::setCurrentPath(const QFileSystemEntry &entry)
{
    ...
    return ::SetCurrentDirectoryA((const char*)(QDir::toNativeSeparators(entry.filePath()).utf8())) != 0;
#else
...
QFileSystemEntry QFileSystemEngine::currentPath()
{
    ...
    if (size != 0) {
        if (size > PATH_MAX) {
            char *newCurrentName = new char[size];
            if (::GetCurrentDirectoryA(PATH_MAX, newCurrentName) != 0)
                ret = QString::fromUtf8(newCurrentName, size);
            delete [] newCurrentName;
        } else {
            ret = QString::fromUtf8(currentName, size);
        }
    }
...
{
    bool ret = 0;
    QFile src(source.filePath());
    QFile dst(target.filePath());
    ...
    if(src.isOpen())
    {
        if(dst.isOpen())
        {
            ...
            {
                QByteArray d = src.read(32768);
                dst.write(d);
            }
            dst.close();
            src.close();
            SetFileAttributesA(target.nativeFilePath().utf8(),
                               GetFileAttributesA(source.nativeFilePath().utf8()));
            ret = 1;
            SetLastError(NO_ERROR);
        }else SetLastError(ERROR_CANTWRITE);
    }else SetLastError(ERROR_CANTREAD);
...
{
    bool ret = ::rename((const char*)source.filePath().utf8(),
                          (const char*)target.filePath().utf8()) == 0;
    if(!ret)
        error = QSystemError(::GetLastError(), QSystemError::NativeError);
    return ret;
}
...
{
    bool ret = ::DeleteFileA((const char*)entry.filePath().utf8()) != 0;
    if(!ret)
        error = QSystemError(::GetLastError(), QSystemError::NativeError);
    return ret;
}
