...
public:
    ...
    explicit QMutex(const char *name, RecursionMode mode = NonRecursive);
    ...
    inline QT3_SUPPORT_CONSTRUCTOR QMutex(bool recursive, const char *name = 0)
    {
        new (this) QMutex(recursive ? Recursive : NonRecursive, name);
    }
#endif
