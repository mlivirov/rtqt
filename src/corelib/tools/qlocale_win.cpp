...
int winGetLocaleInfo(  LCID Locale,       LCTYPE LCType,      LPTSTR lpLCData,
  int cchData       )
{
    return 0;
}
...
QString QSystemLocalePrivate::getLocaleInfo(LCTYPE type, int maxlen)
{
    ...
    if (!winGetLocaleInfo(lcid, type, buf.data(), buf.size()))
        return QString();
...
QSystemLocalePrivate::SubstitutionType QSystemLocalePrivate::substitution()
{
    if (substitutionType == SUnknown) {
        wchar_t buf[8];
        if (!winGetLocaleInfo(lcid, LOCALE_IDIGITSUBSTITUTION, buf, 8)) {
        ...
        if (buf[0] == '1')
            ...
            wchar_t digits[11];
            if (!winGetLocaleInfo(lcid, LOCALE_SNATIVEDIGITS, digits, 11)) {
                substitutionType = QSystemLocalePrivate::SNever;
                return substitutionType;
            }
...
QVariant QSystemLocalePrivate::toString(const QDate &date, QLocale::FormatType type)
{
    ...
    char buf[255];
    if (GetDateFormatA(lcid, flags, &st, NULL, buf, 255)) {
        QString format = QString::fromAscii(buf);
...
QVariant QSystemLocalePrivate::toString(const QTime &time, QLocale::FormatType)
{
    ...
    char buf[255];
    if (GetTimeFormatA(lcid, flags, &st, NULL, buf, 255)) {
        QString format = QString::fromAscii(buf);
        if (substitution() == SAlways)
            substituteDigits(format);
        return format;
...
QVariant QSystemLocalePrivate::measurementSystem()
{
    ...
    if (winGetLocaleInfo(lcid, LOCALE_IMEASURE, output, 2)) {
...
QVariant QSystemLocalePrivate::amText()
{
    ...
    if (winGetLocaleInfo(lcid, LOCALE_S1159, output, 15)) {
...
QVariant QSystemLocalePrivate::pmText()
{
    ...
    if (winGetLocaleInfo(lcid, LOCALE_S2359, output, 15)) {
        return QString::fromWCharArray(output);
    }
...
QVariant QSystemLocalePrivate::firstDayOfWeek()
{
    ...
    if (winGetLocaleInfo(lcid, LOCALE_IFIRSTDAYOFWEEK, output, 4))
...
QVariant QSystemLocalePrivate::currencySymbol(QLocale::CurrencySymbolFormat format)
{
    ...
    case QLocale::CurrencySymbol:
        if (winGetLocaleInfo(lcid, LOCALE_SCURRENCY, buf, 13))
            return QString::fromWCharArray(buf);
        break;
    case QLocale::CurrencyIsoCode:
        if (winGetLocaleInfo(lcid, LOCALE_SINTLSYMBOL, buf, 9))
            return QString::fromWCharArray(buf);
        break;
    case QLocale::CurrencyDisplayName: {
        QVarLengthArray<wchar_t, 64> buf(64);
        if (!winGetLocaleInfo(lcid, LOCALE_SNATIVECURRNAME, buf.data(), buf.size())) {
            if (GetLastError() != ERROR_INSUFFICIENT_BUFFER)
                break;
...
static QString winIso639LangName(LCID id)
{
    ...
    QString lang_code;
    wchar_t out[256];
    if (winGetLocaleInfo(id, LOCALE_ILANGUAGE, out, 255)) // ### shouldn't use them according to msdn
    ...
    if (winGetLocaleInfo(id, LOCALE_SISO639LANGNAME , out, 255))
...
static QString winIso3116CtryName(LCID id)
{
    ...
    wchar_t out[256];
    if (winGetLocaleInfo(id, LOCALE_SISO3166CTRYNAME, out, 255))
        result = QString::fromWCharArray(out);
