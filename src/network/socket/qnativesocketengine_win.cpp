...
#else
        addr = ntohl(sa4->sin_addr.s_un.S_addr);
#endif
        QHostAddress a;
	a.setAddress(addr);
	if (address)
                                               ...
                                               quint16 port, const QHostAddress & address, sockaddr ** sockAddrPtr, QT_SOCKLEN_T *sockAddrSize)
{
    ...
    if (address.protocol() == QAbstractSocket::IPv4Protocol
        ...
        sockAddrIPv4->sin_addr.s_un.S_addr = htonl(address.toIPv4Address());
#endif
...
#else
    SOCKET socket = ::socket(protocol, type, 0);
#endif
...
#if !defined(Q_OS_WINCE) && !defined(ONTIME_RTOS)
...
int QNativeSocketEnginePrivate::option(QNativeSocketEngine::SocketOption opt) const
{
    ...
    case QNativeSocketEngine::NonBlockingSocketOption: {
        unsigned long buf = 0;
#ifdef ONTIME_RTOS
        if(ioctlsocket(socketDescriptor, FIONBIO, &buf) == 0)
#else
...
#else
        if(ioctlsocket(socketDescriptor, FIONBIO, &buf) == SOCKET_ERROR)
#endif
        {
            WS_ERROR_DEBUG(WSAGetLastError());
            return false;
...
bool QNativeSocketEnginePrivate::fetchConnectionParameters()
{
    ...
    qt_sockaddr sa;
    QT_SOCKLEN_T sockAddrSize = sizeof(sa.a4);

    // Determine local address
    memset(&sa, 0, sizeof(sa));
    if (::getsockname(socketDescriptor, (sockaddr *)&sa.a4, &sockAddrSize) == 0) {
    ...
    memset(&sa, 0, sizeof(sa));
    if (::getpeername(socketDescriptor, (sockaddr *)&sa.a4, &sockAddrSize) == 0) {
...
int QNativeSocketEnginePrivate::nativeAccept()
{
    ...
    int acceptedDescriptor = accept(socketDescriptor, (PSOCKADDR)&addr, (PFINT)&addr_size);
#endif
...
qint64 QNativeSocketEnginePrivate::nativeBytesAvailable() const
{
...
#else
    if (::ioctlsocket(socketDescriptor, FIONREAD, &dummy) == SOCKET_ERROR) {
        WS_ERROR_DEBUG(WSAGetLastError());
        return -1;
    }
#endif
    ...
    if (nbytes == 1 && socketType == QAbstractSocket::UdpSocket) {
        ...
        if (::WSARecvFrom(socketDescriptor, &buf, 1, 0, &flags, 0,0,0,0) == SOCKET_ERROR)
            return 0;
#else
        if (::recvfrom(socketDescriptor, &c, 1, MSG_PEEK, 0, 0) == SOCKET_ERROR)
            return 0;
#endif
    }
    return nbytes;
...
#else
    int ioResult = ioctlsocket(socketDescriptor, FIONREAD, &size);
#endif
...
#else
    DWORD bytesRead;
    int wsaRet = ::recvfrom(socketDescriptor, data, maxLength, 0, &aa.a, &sz);
    bytesRead = wsaRet;
#endif
...
#else
    DWORD bytesSent = 0;
    int sendRes = ::sendto(socketDescriptor, data, len, 0, sockAddrPtr, sockAddrSize);
    bytesSent = sendRes;
...
qint64 QNativeSocketEnginePrivate::nativeWrite(const char *data, qint64 len)
{
    ...
    for (;;) {
...
#else
        char *qbuf = (char*)data + ret;
        int qlen = bytesToSend;
        DWORD bytesWritten = 0;
        int socketRet = ::send(socketDescriptor, qbuf, qlen, 0);
        if(socketRet != SOCKET_ERROR)
            bytesWritten = socketRet;
#endif
        ...
        if (socketRet != SOCKET_ERROR) {
            ...
            if (err == WSAENOBUFS) {
#else
            if(err == ENOPKTS || err == EOUTPUTFULL) {
#endif
...
#else
    DWORD bytesRead = 0;
    int recvRes = recv(socketDescriptor, data, maxLength, 0);
    bytesRead = recvRes;
...
//    fds.fd_array[0] = (SOCKET)socketDescriptor;
    FD_SET((SOCKET)socketDescriptor, &fds);
                                      ...
                                      bool *selectForRead, bool *selectForWrite) const
{
    ...
    if (checkRead) {
        FD_SET((SOCKET)socketDescriptor, &fdread);
        //fdread.fd_count = 1;
    ...
    if (checkWrite) {
        ...
        FD_SET((SOCKET)socketDescriptor, &fdwrite);
