...
TEMPLATE = lib
CONFIG  += staticlib
DEFINES += QT_NODLL QT_NO_LIBRARY QT_NO_PROCESS QT_FONTS_ARE_RESOURCES \
            QT_NO_PRINTDIALOG QT_NO_PRINTER \
            QT_QWS_DEPTH_32 QT_QWS_DEPTH_24 QT_QWS_DEPTH_16 QT_QWS_DEPTH_8 \
            QT_NO_SOUND QT_NO_STATEMACHINE QT_FREETYPE\
            QT_NO_QWS_QVFB QT_NO_QWS_LINUXFB QT_NO_QWS_DIRECTFB QT_NO_QWS_TRANSFORMED \
            QT_NO_QWS_VNC QT_NO_QWS_MULTISCREEN QT_NO_QWS_DIRECTFB QT_NO_GLIB \
            QT_NO_IPV6 QT_NO_SXE QT_NO_QWS_MULTIPROCESS QT_SQL_SQLITE


decorations = styled windows default
styles = windows plastique cleanlooks
QT_CONFIG += jpeg tiff gif freetype
########################################################
DEFINES += ONTIME_RTOS
DEFINES += QT_NEWPORT
########################################################
isEmpty(QT_SOURCE_TREE){
            ...
            QMAKE_INCDIR_QT = $$QT_SOURCE_TREE/include
}

contains(DEFINES, ONTIME_RTOS) {
    DEFINES += Q_OS_ONTIME_RTOS
    QTRT_SOURCE_TREE = c:/rtos
    QMAKE_LFLAGS += /NODEFAULTLIB:"kernel32.lib"
    #LIBS+= -L$$QTRT_SOURCE_TREE/libmsvc rtk32.lib

    INCLUDEPATH += $$QTRT_SOURCE_TREE/include
}


contains(DEFINES, QT_FREETYPE) {
            QT_FREETYPE_TREE = $$QT_SOURCE_TREE/src/3rdparty/freetype
            INCLUDEPATH += $$QT_FREETYPE_TREE/include
            INCLUDEPATH += $$QT_FREETYPE_TREE/include/freetype
}
########################################################
