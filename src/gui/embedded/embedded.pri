...
embedded|contains(DEFINES, ONTIME_RTOS) {
	CONFIG -= opengl x11
	LIBS -= -dl
	KERNEL_P        = kernel
...
#
        contains(DEFINES, ONTIME_RTOS) {
            HEADERS += embedded/qscreenITForOntime.h \
                        embedded/qrtoskbd.h \
                        embedded/qrtosmousehandler.h

            SOURCES += embedded/qscreenITForOntime.cpp \
                        embedded/qrtoskbd.cpp \
                        embedded/qrtosmousehandler.cpp \
                        embedded/qrtosinput.cpp
        }
