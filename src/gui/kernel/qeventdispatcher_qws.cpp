...
#define PLATFORM_EVENT_DISPATCHER_PRIVATE QEventDispatcherUNIX
#else
#include "private/qeventdispatcher_win_p.h"
#define PLATFORM_EVENT_DISPATCHER_PRIVATE QEventDispatcherWin32Private
#endif
...
class QEventDispatcherQWSPrivate : public PLATFORM_EVENT_DISPATCHER_PRIVATE
{
...
QEventDispatcherQWS::QEventDispatcherQWS(QObject *parent)
    : PLATFORM_EVENT_DISPATCHER(*new QEventDispatcherQWSPrivate, parent)
{
}
...
bool QEventDispatcherQWS::processEvents(QEventLoop::ProcessEventsFlags flags)
{
    ...
    if (!d->interrupt) {
		...
		if (PLATFORM_EVENT_DISPATCHER::processEvents(flags))
            return true;
    }
