...
void QFontEngineQPF::remapFontData()
{
    ...
    {
        if(fontData) ::free((void*)fontData);
        fontData = (const uchar*)malloc(newFileSize);
        ::lseek(fd, 0, SEEK_SET);
        uchar *pd = (uchar *)fontData;
        while(pd && !::eof(fd))
        {
            int res = ::read(fd, pd, 1024);
            if(res > 0) pd+=res;
            else
            {
                free((void *)fontData);
        ...
        if (!fontData || fontData == (const uchar *)0) {
#  if defined(DEBUG_FONTENGINE)
            perror("mmap failed");
#  endif
            fontData = 0;
